                ...
                <form @submit.prevent="addStaff">
                    <div class="grid grid-cols-6 gap-6">
                        <div class="col-span-6 sm:col-span-3">
                            ...
                            <input v-model="formData.MSNV" type="text" name="product-name" id="product-name"
                                class="shadow-sm bg-gray-50 border border-gray-300 text-gray-900 sm:text-sm rounded-lg focus:ring-cyan-600 focus:border-cyan-600 block w-full p-2.5"
                                required="">
                        </div>
                        <div class="col-span-6 sm:col-span-3">
                            <label for="category" class="text-sm font-medium text-gray-900 block mb-2">Họ Tên</label>
                            <input v-model="formData.HoTenNV" type="text" name="category" id="category"
                                class="shadow-sm bg-gray-50 border border-gray-300 text-gray-900 sm:text-sm rounded-lg focus:ring-cyan-600 focus:border-cyan-600 block w-full p-2.5"
                                required="">
                        </div>
                        <div class="col-span-6 sm:col-span-3">
                            <label for="price" class="text-sm font-medium text-gray-900 block mb-2">Chức vụ</label>
                            <input v-if="formData.ChucVu === 'Staff'" v-model="formData.ChucVu" type="text" name="price"
                                id="price"
                                class="shadow-sm bg-gray-50 border border-gray-300 text-gray-900 sm:text-sm rounded-lg focus:ring-cyan-600 focus:border-cyan-600 block w-full p-2.5"
                                required>
                            <select v-else v-model="formData.ChucVu"
                                class="shadow-sm bg-gray-50 border border-gray-300 text-gray-900 sm:text-sm rounded-lg focus:ring-cyan-600 focus:border-cyan-600 block w-full p-2.5"
                                required>
                        ...
                        <div class="col-span-6 sm:col-span-3">
                            ...
                            <input v-model="formData.DiaChi" type="text" name="category" id="category"
                                class="shadow-sm bg-gray-50 border border-gray-300 text-gray-900 sm:text-sm rounded-lg focus:ring-cyan-600 focus:border-cyan-600 block w-full p-2.5"
                                required="">
                        </div>
                        <div class="col-span-6 sm:col-span-3">
                            <label for="category" class="text-sm font-medium text-gray-900 block mb-2">SDT</label>
                            <input v-model="formData.SDT" type="number" name="category" id="category"
                                class="shadow-sm bg-gray-50 border border-gray-300 text-gray-900 sm:text-sm rounded-lg focus:ring-cyan-600 focus:border-cyan-600 block w-full p-2.5"
                                required="">
                        </div>
                        <div class="col-span-6 sm:col-span-3">
                            <label for="category" class="text-sm font-medium text-gray-900 block mb-2">Mật khẩu</label>
                            <input v-model="formData.Password" type="password" name="category" id="category"
                                class="shadow-sm bg-gray-50 border border-gray-300 text-gray-900 sm:text-sm rounded-lg focus:ring-cyan-600 focus:border-cyan-600 block w-full p-2.5"
                                required="">
                        </div>
                    </div>
...
>Xoá</button>`;
        ...
        api.delete(`/staff/delete/${_id}`)
            .then(response => {
                // Xử lý khi xóa thành công
                console.log('Staff đã được xóa:', response.data);
                this.fetchStaffs();
                // Sau khi xóa thành công, làm mới DataTable để cập nhật danh sách sách
                if (this.isDataTableInitialized) {
                    this.staffTable.clear().rows.add(response.data.data).draw();
                }
            })
            .catch(error => {
                // Xử lý khi xóa không thành công
                console.error('Lỗi khi xóa staff:', error);
                this.errorMessage = "Lỗi khi xóa staff: " + error.message;
            });
    },
    },
